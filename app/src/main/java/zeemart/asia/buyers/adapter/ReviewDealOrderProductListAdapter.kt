 ...
 */
class ReviewDealOrderProductListAdapter(
    private val context: Context,
    private val reviewOrderItemChangeListener: ReviewDealOrderItemChangeListener,
    private val products: MutableList<Product>,
    private val supplierDetails: Supplier
) : RecyclerView.Adapter<ReviewDealOrderProductListAdapter.ViewHolder>(),
            ...
            if (!products[position].isProductManuallyAdded) {
                if (dialog == null || dialog != null && !dialog!!.isShowing) {
                    val createOrderHelperDialog = DealProductQuantityChangeDialogHelper(
                        products[position], context, this@ReviewDealOrderProductListAdapter
                    )
    ...
    private fun setPlaceOrderButtonState() {
        ...
        if (isActive) {
            ...
            (context as ReviewDealsActivity).calculateTotalPrice()
        } else {
            (context as ReviewDealsActivity).calculateTotalPrice()
        }
    }
    ...
    interface ReviewDealOrderItemChangeListener {
