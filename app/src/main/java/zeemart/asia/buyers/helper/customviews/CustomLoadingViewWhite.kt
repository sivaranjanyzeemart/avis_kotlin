...
class CustomLoadingViewWhite(private val context: Context, attrs: AttributeSet?) : LinearLayout(
    context, attrs
) {
    private var bgColor = 0

    init {
        val a = context.obtainStyledAttributes(attrs, R.styleable.overlay_view)
        bgColor = try {
            a.getColor(R.styleable.overlay_view_loader_overlay_background, 0)
        } finally {
            a.recycle()
        }
        init(context)
    ...
    private fun init(context: Context) {
        val rootView = inflate(context, R.layout.lyt_three_dot_loader_white, this)
        val mainLayout = rootView.findViewById<ConstraintLayout>(R.id.lyt_white_loader_white)
        if (bgColor != 0) {
            mainLayout.setBackgroundColor(bgColor)
        }
        mainLayout.setOnClickListener {
            //view behind should not be clickable once this view is visible.
        }
